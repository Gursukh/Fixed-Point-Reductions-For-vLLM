...
reproducible. The trick is simple: cast to a signed fixed-point integer before
reducing, accumulate in integer space, then cast back. Integer addition is
associative and commutative, so the result stops depending on how the reduction
is split across SMs, warps, or KV splits. You get the same logits regardless of
batch composition, tensor-parallel degree, or kernel tiling - the usual causes
of run-to-run drift in LLM inference.

The plugin swaps out the reductions that affect determinism - the GEMMs
(including the LM head), RMSNorm, and attention - for fixed-point variants, and
registers them with vLLM through its standard quantization and attention
backend hooks.
...
The fixed-point GEMM uses tensor cores and needs compute capability sm_75+
(Turing) for fp16; the bf16 and fp32 paths also need sm_80+ (Ampere).
...
point, so there's no init call to make. Setting `quantization` picks the
fixed-point quant config (which routes the GEMMs and LM head), and
`attention_backend="CUSTOM"` picks the deterministic attention backend. The
RMSNorm patch is applied at registration time and is on by default.

### Configuration

Everything is configured through environment variables, read once at process
start; see [`config.py`](fxpr_vllm/config.py). The defaults are fully
deterministic out of the box.

| Variable                         | Default | Allowed       | Effect |
| -------------------------------- | ------- | ------------- | ------ |
| `FXPR_INT_BITS`                  | `32`    | `16`/`32`/`64`| Width of the integer accumulator. |
| `FXPR_FRAC_BITS`                 | `16`    | `8`/`16`/`32` | Fractional bits in the Q-format. More bits trade dynamic range for finer resolution. |
| `FXPR_DISABLE_RMS_NORM`          | unset   | `1`           | Skip the `DeterministicRMSNorm` patch and keep vLLM's stock RMSNorm. |
...
The `int_bits` / `frac_bits` pair sets the representable range. Q16.16 (that is,
`FXPR_FRAC_BITS=16` in a 32-bit accumulator) covers roughly +/-32K in the
original units before it saturates. Bump `FXPR_INT_BITS` to 64 if your
...
The suite lives in `fxpr_vllm/_tests/` (the pytest `testpaths`) and needs CUDA.
The bf16/fp32 GEMM tests need sm_80+ (Ampere or newer) and skip on older
hardware.
