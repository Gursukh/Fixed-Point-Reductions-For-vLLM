"""torch.library-registered wrappers around the fixed-point Triton kernels.

Inductor sees one opaque graph node per call instead of tracing through the
Python launch wrapper. Drastically reduces FX graph size when many Linear /
RMSNorm calls go through these kernels under torch.compile.
"""
